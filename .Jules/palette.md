...
## 2026-06-09 - HTML5 Semantics and Child Elements
**Learning:** Heading elements (`<h2>`, etc.) in HTML5 only permit phrasing content (like `<span>`), not flow content (like `<div>`). Converting a structural container `<div>` into a heading element while leaving `<div>` children inside results in invalid HTML which can cause unpredictable behavior for screen readers.
**Action:** When converting generic containers to semantic headings, ensure all child elements are updated to valid phrasing content tags (e.g., changing `<div>` to `<span>`) to maintain valid HTML5 syntax.
